...
run_docker () {
  podman run --rm --name consul -p 8500:8500 -p 8501:8501 consul:1.7 agent -dev -ui -client=0.0.0.0 -bind=0.0.0.0 --https-port=8501 &
  pids[$1]=$!
}
...
app_available () {
  if curl -I localhost:8500/ui/ | grep -q "200 OK" && curl -I localhost:8085 | grep -q "200 OK" &&
    ...
    curl -I localhost:8084 | grep -q "200 OK" && curl -I localhost:8080/eshop-rest/users | grep -q "200 OK"; then
      return 0
  else
      return 1
  fi
